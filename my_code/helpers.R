...
plot_totscore_over_time_old <- function(ds, lastname = NA_character_,
                                    sum_by_round = FALSE) {
...
plot_totscore_over_time <- function (ds, lastname = NA) {
  ...
  # If `lastname` is supplied; return plot for single player
  if (!is.na(lastname)) {
    d_plot <- filter(d_plot, player == lastname)
  }
